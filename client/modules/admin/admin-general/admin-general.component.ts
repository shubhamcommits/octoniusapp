  ...
  implements OnInit, AfterContentChecked, OnDestroy
{
  ...
  async ngOnInit() {
    ...
    this.allowDecentralizedRoles =
      this.workspaceData["allow_decentralized_roles"];
  ...
  saveSettings(selected) {
    ...
    this.utilityService.asyncNotification(
      $localize`:@@adminGeneral.pleaseWaitsavingSettings:Please wait, we are saving the new setting...`,
      new Promise((resolve, reject) => {
        this.workspaceService
          .saveSettings(this.workspaceData["_id"], {
            allow_decentralized_roles: selected.checked,
          })
          .then(() => {
            ...
            resolve(
              this.utilityService.resolveAsyncPromise(
                $localize`:@@adminGeneral.settingsSaved:Settings saved!`
              )
            );
          })
          .catch(() =>
            reject(
              this.utilityService.rejectAsyncPromise(
                $localize`:@@adminGeneral.unableToSaveGroupSettings:Unable to save the settings, please try again!`
              )
            )
          );
      })
