import { Component, OnInit, Injector, Input, OnChanges, AfterViewInit, SimpleChanges, Output, EventEmitter, OnDestroy } from '@angular/core';
...
@Component({
  selector: 'app-group-kanban-archived-boards',
  templateUrl: './group-kanban-archived-boards.component.html',
  styleUrls: ['./group-kanban-archived-boards.component.scss']
})
export class GroupKanbanArchivedBoardsComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  async initView() {
     ...
     */
    this.columns = await this.publicFunctions.getAllArchivedColumns(this.groupData?._id);
    ...
    this.tasks = await this.publicFunctions.getArchivedTasks(this.groupData?._id);
  ...
  removeColumn(columnId: string) {
    ...
    utilityService.asyncNotification($localize`:@@groupKanbanArchivedBoards.pleaseWaitWeRemovingSection:Please wait we are removing your section...`, new Promise((resolve, reject) => {
  ...
  editColumn(oldCol: any, newColTitle: any) {
    ...
    // If not found, then change the element details
      ...
      // Call the HTTP Service function
      this.utilityService.asyncNotification($localize`:@@groupKanbanArchivedBoards.pleaseWaitWeRenameSection:Please wait we are renaming your section`, new Promise((resolve, reject) => {
        columnService.editColumnName(columnId, newColTitle)
          .then((res) => {
            resolve(this.utilityService.resolveAsyncPromise($localize`:@@groupKanbanArchivedBoards.sectionRenamed:Section Renamed!`));
          })
          .catch((err) => {
            reject(this.utilityService.rejectAsyncPromise($localize`:@@groupKanbanArchivedBoards.unableToRenameSection:Unable to rename the section at the moment, please try again!`))
          })
      }))
    }
  }
  ...
  deleteColumn(column: any) {
    ...
    this.utilityService.getConfirmDialogAlert($localize`:@@groupKanbanArchivedBoards.areYouSure:Are you sure?`, $localize`:@@groupKanbanArchivedBoards.byDoingThisTasksWillBeDeleted:By doing this all the tasks from this section will be deleted!`)
      .then((res) => {
        if (res.value) {
          // Find the index of the column to check if the same named column exist or not
  ...
  unarchiveColumn(column: any) {
    ...
    // Open the Confirm Dialog to ask for permission
    this.utilityService.getConfirmDialogAlert($localize`:@@groupKanbanArchivedBoards.areYouSure:Are you sure?`, $localize`:@@groupKanbanArchivedBoards.byDoingThisTasksWillBeUnarchived:By doing this all the tasks from this section will be unarchive!`)
      .then((res) => {
        if (res.value) {
          // Find the index of the column to check if the same named column exist or not
          let index = this.columns.findIndex((col: any) => col._id === column._id);
          ...
          // Call the HTTP Service function
          utilityService.asyncNotification($localize`:@@groupKanbanArchivedBoards.pleaseWaitWeUnarchivingSection:Please wait we are unarchiving your section...`, new Promise((resolve, reject) => {
            columnService.archiveColumn(column._id)
              .then((res) => {
                // Remove the column from the array
                this.columns.splice(index, 1);

                resolve(utilityService.resolveAsyncPromise($localize`:@@groupKanbanArchivedBoards.sectionUnarchived:Section Unarchived!`));
              })
              .catch((err) => {
                reject(utilityService.rejectAsyncPromise($localize`:@@groupKanbanArchivedBoards.unableToUnarchiveSection:Unable to unarchived the section at the moment, please try again!`))
              })
          }))
        }
      })
