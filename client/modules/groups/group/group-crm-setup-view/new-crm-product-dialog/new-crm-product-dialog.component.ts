...
export class NewCRMProductDialogComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.productData._group = this.groupData;

    if (!!this.data && !!this.data.productId) {
      await this.crmGroupService.getCRMProduct(this.data.productId).then(res => {
  ...
  saveProduct() {
    if (!!this.productData?._id) {
      this.crmGroupService.updateCRMProduct(this.productData, this.groupData?._id).then(res => {
        this.productData = res['product'];
        this.productEdited.emit(this.productData);
      });
    } else {
      this.crmGroupService.createCRMProduct(this.productData).then(res => {
        this.productData = res['product'];
