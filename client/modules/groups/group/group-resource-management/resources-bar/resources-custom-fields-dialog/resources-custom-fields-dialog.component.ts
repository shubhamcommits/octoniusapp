...
@Component({
  selector: 'app-resources-custom-fields-dialog',
  templateUrl: './resources-custom-fields-dialog.component.html',
  styleUrls: ['./resources-custom-fields-dialog.component.scss']
})
export class ResourcesCustomFieldsDialogComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.resourcesGroupService.getGroupResourcesCustomFields(this.groupData._id).then((res) => {
      if (res['group']['resources_custom_fields']) {
        res['group']['resources_custom_fields'].forEach(async field => {
  ...
  async createCustomField() {
    if (this.newCustomFieldTitle !== '') {
      ...
      if (index !== -1) {
        this.utilityService.warningNotification($localize`:@@resourcesCustomFieldDialog.fieldAlreadyExist:Field already exist!`);
      } else {
        const newCF = {
        ...
        // Save the new field
        await this.resourcesGroupService.saveNewResourcesCustomField(newCF, this.groupData._id).then((res) => {
          const dbCustomFields = res['group']['resources_custom_fields']
          const index = dbCustomFields.findIndex((f: any) => f.name.toLowerCase() === newCF.name);
          if (index >= 0) {
            newCF['_id'] = dbCustomFields[index]._id;
          }
          this.groupData.resources_custom_fields = res['group'].resources_custom_fields;
  ...
  removeCustomField(field) {
    ...
    this.utilityService.getConfirmDialogAlert()
      .then((result) => {
        if (result.value) {
          // Remove the file
          this.utilityService.asyncNotification($localize`:@@resourcesCustomFieldDialog.pleaseWaitDeletingCF:Please wait we are deleting the custom field...`, new Promise((resolve, reject) => {
            const index = this.customFields.findIndex((f: any) => f.name.toLowerCase() === field.name.toLowerCase());
            if (index !== -1) {
              // Remove the value
              this.resourcesGroupService.removeResourcesCustomField(field._id, this.groupData._id)
                .then((res) => {
                  ...
                  resolve(this.utilityService.resolveAsyncPromise($localize`:@@resourcesCustomFieldDialog.fieldDeleted:Field deleted!`));
                }).catch((err) => {
                  reject(this.utilityService.rejectAsyncPromise($localize`:@@resourcesCustomFieldDialog.unableToDeleteField:Unable to delete field, please try again!`));
                });
            }
          }));
        }
  ...
  async addValue(field, event: Event) {
    ...
    if (newValue !== '') {
      ...
      // If index is found, then throw error notification
      if (index !== -1) {
        this.utilityService.warningNotification($localize`:@@resourcesCustomFieldDialog.valueAlreadyExists:Value already exists!`);
      } else {
        field.values.push(newValue);
        await this.sortValues(field);

        // Save the new value
        this.resourcesGroupService.addResourcesCustomFieldNewValue(newValue, field._id, this.groupData._id).then(res => {
          this.groupData.resources_custom_fields = res['group'].resources_custom_fields;
          this.publicFunctions.sendUpdatesToGroupData(this.groupData);
        });
  ...
  //   this.utilityService.asyncNotification($localize`:@@resourcesCustomFieldDialog.pleaseWaitUpdatingCF:Please wait we are updating the custom field...`, new Promise((resolve, reject) => {
  //     this.resourcesGroupService.setResourcesCustomFieldDisplayKanbanCard(!field.display_in_kanban_card, field._id, this.groupData._id).then(res => {
  //       this.groupData.resources_custom_fields = res['group'].resources_custom_fields;
  //       this.publicFunctions.sendUpdatesToGroupData(this.groupData);

  //       resolve(this.utilityService.resolveAsyncPromise($localize`:@@resourcesCustomFieldDialog.fieldUpdated:Field updated!`));
  //     }).catch((err) => {
  //       reject(this.utilityService.rejectAsyncPromise($localize`:@@resourcesCustomFieldDialog.unableToUpdateField:Unable to update field, please try again!`));
  //     });
  //   }));
  // }
  ...
  removeValue(field, value: string) {
    ...
    if (index !== -1) {
      // Remove the value
      this.resourcesGroupService.removeResourcesCustomFieldValue(value, field._id, this.groupData._id)
        .then((res) => {
  ...
  openColorPicker(field: any) {
    ...
    const colorPickedSubs = dialogRef.componentInstance.colorPickedEvent.subscribe(async (data) => {
      ...
      this.utilityService.asyncNotification($localize`:@@resourcesCustomFieldDialog.pleaseWaitUpdatingCF:Please wait we are updating the custom field...`, new Promise((resolve, reject) => {
        this.resourcesGroupService.setResourcesCustomFieldColor(data, field._id, this.groupData._id).then(res => {
  ...
  removeBadgeColor(field: any) {
    ...
    this.utilityService.asyncNotification($localize`:@@resourcesCustomFieldDialog.pleaseWaitUpdatingCF:Please wait we are updating the custom field...`, new Promise((resolve, reject) => {
      this.resourcesGroupService.setResourcesCustomFieldColor('', field._id, this.groupData._id).then(res => {
        this.groupData.resources_custom_fields = res['group'].resources_custom_fields;
