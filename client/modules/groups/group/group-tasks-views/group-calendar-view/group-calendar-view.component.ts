...
export class GroupCalendarViewComponent implements OnInit {
  ...
  actions: CalendarEventAction[] = [
    ...
    },
    {
      ...
      onClick: ({ event }: { event: CalendarEvent }): void => {
        ...
        this.handleEvent(
          $localize`:@@groupCalendarView.deleted:Deleted`,
          event
        );
      },
    },
  ...
  dayClicked({ date, events }: { date: Date; events: CalendarEvent[] }): void {
    if (
      this.datesService.isSameMonth(
        ...
        DateTime.fromJSDate(this.viewDate)
      )
    ) {
      if (
  ...
  eventTimesChanged({
    ...
    this.handleEvent(
      $localize`:@@groupCalendarView.droppedResized:Dropped or resized`,
      event
    );
  }
