...
export class TasksTableComponent implements OnChanges, AfterViewInit {
  ...
  sortNSValues() {
    this.tasks?.forEach((post) => {
      if (!!post?.task?.northStar?.values) {
        post.task.northStar.values = post?.task?.northStar?.values?.sort(
          (v1, v2) => (this.datesService.isBefore(v1.date, v2.date) ? 1 : -1)
        );
      }
    });
  }
  ...
  loadCustomFieldsToShow() {
    if (
      this.section &&
      this.section.custom_fields_to_show &&
      this.customFieldsToShow.length === 0
    ) {
      this.section.custom_fields_to_show.forEach((field) => {
  ...
  getProgressPercent(northStar) {
    northStar.values = northStar?.values?.sort((v1, v2) =>
  ...
  openFullscreenModal(postData: any): void {
    ...
    if (dialogRef) {
      ...
      const datesChangeEventSub =
        dialogRef.componentInstance.datesChangeEvent.subscribe(async (data) => {
  ...
  async updateTask(post: any, cfTrigger?: any) {
    if (post) {
      ...
      await this.publicFunctions
        .executedAutomationFlowsPropertiesFront(null, post, this.groupData?._id)
        .then((res) => {
          post = res;
  ...
  addNewColumn($event: Event) {
    ...
    const index = this.customFieldsToShow.findIndex(
      (f: any) =>
        f.name.toLowerCase() === this.newColumnSelected.name.toLowerCase()
    );

    // If index is found, then throw error notification
    if (index !== -1) {
      this.utilityService.warningNotification(
        $localize`:@@taskTable.sectionAlreadyExists:Section already exists!`
      );
    } else {
      ...
      this.customFieldsToShow.push(
        this.getCustomField(this.newColumnSelected.name)
      );
      if (this.displayedColumns.length - 1 >= 0) {
        this.displayedColumns.splice(
          this.displayedColumns.length - 1,
          0,
          this.newColumnSelected.name
        );
      }

      this.newColumnSelected = null;

      this.columnService.saveCustomFieldsToShow(
        this.section._id,
        this.section.custom_fields_to_show
      );
    }
  ...
  removeColumn(field: any) {
    let index: number = this.customFieldsToShow.findIndex(
      (cf) => cf.name === field
    );
    if (index !== -1) {
  ...
  getTaskClass(
    ...
    if (task.task.shuttle_type) {
      const shuttleIndex = task.task.shuttles
        ? task.task.shuttles.findIndex(
            (s) =>
      ...
      const shuttleStatus =
        shuttleIndex >= 0
          ? task.task.shuttles[shuttleIndex].shuttle_status
          : status;
