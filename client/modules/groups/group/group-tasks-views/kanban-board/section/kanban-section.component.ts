...
export class KanbanSectionComponent implements OnChanges, OnDestroy {
  ...
  constructor(
    ...
    this.subSink.add(
      this.columnService.refresh$.subscribe((data) => {
        if (
          !!data &&
          !!this.section &&
          (data?.sectionId == this.section?._id ||
            data?.oldSectionId == this.section?._id)
        ) {
          this.initSection();
        }
      })
  ...
  async ngOnChanges(changes: SimpleChanges) {
    ...
    for (const propName in changes) {
      ...
      if (propName === "filteringBit" || propName === "filteringData") {
        this.tasks = await this.postService.filterTasks(
          this.tasks,
          this.filteringBit,
          this.filteringData,
          this.userData
  ...
  async initSection() {
    if (!!this.section && !!this.section._id) {
      this.tasks = await this.postService.getTasksBySectionPromise(
        this.section?._id,
        this.isShuttleTasksModuleAvailable
      );

      if (this.groupData?.enabled_rights) {
        this.tasks = await this.postService.filterRAGTasks(
          this.unchangedTasks?.tasksList,
          this.groupData,
          this.userData
        );
      }
  ...
  drop(event: CdkDragDrop<string[]>) {
    const oldColumn =
      this.sections[
        this.sections.findIndex((sec) => sec._id == event.previousContainer.id)
      ];
    const newColumn =
      this.sections[
        this.sections.findIndex((sec) => sec._id == event.container.id)
      ];
    if (oldColumn && newColumn && oldColumn.canEdit && newColumn.canEdit) {
      if (event.previousContainer === event.container) {
        ...
        this.moveTaskToSection(
          event.previousContainer.data[event.previousIndex],
          event.container.id,
          oldColumn._id /*, shuttleIndex, event.previousContainer.data, event.container.data, event.previousIndex, event.currentIndex*/
        );
      }
    }
  }
  ...
  removeTask(post: any) {
    ...
    let index = this.tasks
      ? this.tasks.findIndex((task: any) => task._id == post._id)
      : -1;
  ...
  async moveTaskToSection(
    post: any,
    sectionId: string,
    oldSectionId: string /*, shuttleIndex: number, previousData, currentData, previousIndex, currentIndex*/
  ) {
    const shuttleIndex =
      !!post && !!post.task && !!post.task.shuttles
    ...
    if (post?.task?.shuttle_type && shuttleIndex >= 0) {
      ...
      await this.publicFunctions
        .changeTaskShuttleSection(post?._id, this.groupData?._id, sectionId)
        .then(async (res) => {
          await this.updateTaskSectionFront(
            post,
            shuttleIndex,
            sectionId,
            oldSectionId
  ...
  changeTaskColumn(
    ...
    this.utilityService.asyncNotification(
      $localize`:@@kanbanSection.pleaseWaitWeAreMovingTaskToSection:Please wait we are moving the task to a new section...`,
          ...
          .then(async (res) => {
            ...
            resolve(
              this.utilityService.resolveAsyncPromise(
                $localize`:@@kanbanSection.tasksMoved:Task moved`
              )
            );
          })
          .catch(() => {
            reject(
              this.utilityService.rejectAsyncPromise(
                $localize`:@@kanbanSection.unableToMoveTask:Unable to move the task, please try again!`
              )
            );
          });
      })
  ...
  async updateTask(post: any) {
    ...
    if (indexTask >= 0) {
      if (
        ...
        const indexShuttle = post.task.shuttles.findIndex(
          (shuttle) =>
            this.section._id ==
            (shuttle._shuttle_section._id || shuttle._shuttle_section)
        );
        if (indexShuttle >= 0) {
          // update the tasks from the array
          this.tasks[indexTask] = post;
  ...
  makeColumnProjectDialog(column: any) {
    if (!column?.project_type) {
      this.utilityService.asyncNotification(
        $localize`:@@kanbanSection.pleaseWaitWeCreateProject:Please wait we are creating a project from your section...`,
        new Promise((resolve, reject) => {
          this.columnService
            .changeColumnProjectType(column._id, true)
            .then((res) => {
              column.project_type = true;
              resolve(
                this.utilityService.resolveAsyncPromise(
                  $localize`:@@kanbanSection.sectionTypeChanged:Section type changed!`
                )
              );
            })
            .catch((err) => {
              ...
              reject(
                this.utilityService.rejectAsyncPromise(
                  $localize`:@@kanbanSection.unableToChangeSectionType:Unable to change the section type at the moment, please try again!`
                )
              );
            });
        })
  ...
  openShowCFDialog(column) {
    ...
    const customFieldsUpdatedEventSubs =
      dialogRef?.componentInstance?.customFieldsUpdatedEvent.subscribe(
        (data) => {
          this.section = data;
        }
      );
  ...
  openPermissionModal(item: any): void {
    const dialogRef = this.utilityService.openPermissionModal(
      item,
      this.groupData,
      this.userData,
      "section"
  ...
  openFullscreenModal(postData: any): void {
    ...
    if (dialogRef) {
      const deleteEventSubs =
        dialogRef?.componentInstance?.deleteEvent?.subscribe(async (data) => {
      ...
      const taskClonnedEventSubs =
        dialogRef?.componentInstance?.taskClonnedEvent?.subscribe(
          async (data) => {
      ...
      const sectionChangedEventSubs =
        dialogRef?.componentInstance?.sectionChangedEvent?.subscribe(
          async (data) => {
            const sectionId = data.task._column._id || data.task._column;
            const oldSectionId =
              postData.task._column._id || postData.task._column;
            await this.columnService.triggerRefreshSection({
              sectionId,
              oldSectionId,
            });
      ...
      const datesChangeEventSub =
        dialogRef?.componentInstance?.datesChangeEvent?.subscribe(
          async (data) => {
  ...
  onDeleteEvent(id) {
    ...
    const indexTask = this.tasks
      ? this.tasks.findIndex((task: any) => task._id === id)
      : -1;
    if (indexTask !== -1) {
      this.tasks.splice(indexTask, 1);
      return;
  ...
  isDelay(realDueDate: any, dueDate: any) {
    return this.datesService.isBefore(
      DateTime.fromISO(dueDate),
      DateTime.fromISO(realDueDate)
    );
  }
  ...
  countDoneTasks() {
    this.section.numDoneTasks = this.tasks.filter(
