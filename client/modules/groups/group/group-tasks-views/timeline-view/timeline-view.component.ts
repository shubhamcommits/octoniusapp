import { Component, Input, OnInit, ViewChild, ElementRef, AfterViewInit, Output, EventEmitter, Injector, OnChanges, Renderer2 } from '@angular/core';
...
export class TimelineViewComponent implements OnChanges, AfterViewInit {
  ...
  async initView() {
    ...
    if (this.groupData?.enabled_rights) {
      await this.postService.filterRAGTasks(this.tasks, this.groupData, this.userData);
    }
    ...
    if (!!sections) {
      sections.forEach(async (section: any) => {
        ...
        section.tasks = await tasks
          .filter((post: any) => ((post.task._column && (post.task._column._id || post.task._column) == section._id)
            || (post.task.shuttle_type && post.task.shuttles
                && post.task.shuttles.findIndex(shuttle => (shuttle._shuttle_section._id || shuttle._shuttle_section) == section._id) >= 0))
          )
          .sort(function(t1, t2) {
        ...
        section.numDoneTasks = section.tasks.filter((post) => post?.task?.status?.toLowerCase() == 'done').length;
      });
    }
  }
  ...
  linesGenetate() {
    setTimeout(() => {
      ...
      document.getElementById('fixed-container-timeline')?.addEventListener('scroll', this.linePotionsListener.bind(this), false);
    }, 50);
  };
  ...
  lineRemove() {
    document.getElementById('fixed-container-timeline')?.removeEventListener('scroll', this.linePotionsListener.bind(this), false);
    this.linesArray.forEach(line => {
      line.remove();
    });
    this.linesArray = [];
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    if (projectId) {
      var newEndDate = DateTime.fromISO(this.projectsdata[projectId].tasks[Taskindex].end).plus({days: days});
      var newStartDate = DateTime.fromISO(this.projectsdata[projectId].tasks[Taskindex].start).plus({days: days});
      ...
      this.projectsdata[projectId].tasks[Taskindex].task.task.due_to = newEndDate;
      this.projectsdata[projectId].tasks[Taskindex].task.task.start_date = newStartDate;
      this.projectsdata[projectId].tasks[Taskindex].start = newStartDate;
      this.dateupdate(this.projectsdata[projectId].tasks[Taskindex], newStartDate, newEndDate, days, days, this.projectsdata[projectId].tasks[Taskindex]?._groupid);

    } else {
      var newEndDate = DateTime.fromISO(this.tasksDataList[Taskindex].end).plus({days: days});
      var newStartDate = DateTime.fromISO(this.tasksDataList[Taskindex].start).plus({days: days});
      ...
      this.dateupdate(this.tasksDataList[Taskindex], newStartDate, newEndDate, days, days, this.tasksDataList[Taskindex]?._groupid);
    }
    setTimeout(() => {
      this.linesArray.forEach(line => {
        line.position();
  ...
  onResizeEnd(event: ResizeEvent, Taskid: string, Taskindex:number,projectIndex?:number): void {
    ...
    if (event.edges?.right) {
      ...
      if(projectIndex>=0){
        var newEndDate = DateTime.fromISO(this.projectsdata[projectIndex].tasks[Taskindex].end).plus({ days: multiple });
        this.projectsdata[projectIndex].tasks[Taskindex].end = newEndDate;
        this.projectsdata[projectIndex].tasks[Taskindex].task.task.due_to = newEndDate;
        this.dateupdate(this.projectsdata[projectIndex].tasks[Taskindex], this.projectsdata[projectIndex].tasks[Taskindex].start, newEndDate, 0, multiple, this.projectsdata[projectIndex].tasks[Taskindex]?._groupid);

      }else{
        var newEndDate = DateTime.fromISO(this.tasksDataList[Taskindex].end).plus({ days: multiple });
        this.tasksDataList[Taskindex].end = newEndDate;
        this.tasksDataList[Taskindex].task.task.due_to = newEndDate;
        this.dateupdate(this.tasksDataList[Taskindex], this.tasksDataList[Taskindex].start, newEndDate, 0, multiple, this.tasksDataList[Taskindex]?._groupid);
      }
      ...
      if(projectIndex>=0){
        var newStartDate = DateTime.fromISO(this.projectsdata[projectIndex].tasks[Taskindex].start).plus({ days: multiple });
        this.projectsdata[projectIndex].tasks[Taskindex].task.task.start_date = newStartDate;
        this.projectsdata[projectIndex].tasks[Taskindex].start = newStartDate;
        this.dateupdate(this.projectsdata[projectIndex].tasks[Taskindex], newStartDate, this.projectsdata[projectIndex].tasks[Taskindex].end, multiple, 0, this.projectsdata[projectIndex].tasks[Taskindex]?._groupid);
      }else{
        var newStartDate = DateTime.fromISO(this.tasksDataList[Taskindex].start).plus({ days: multiple });
        this.tasksDataList[Taskindex].task.task.start_date = newStartDate;
        this.tasksDataList[Taskindex].start = newStartDate;
        this.dateupdate(this.tasksDataList[Taskindex], newStartDate, this.tasksDataList[Taskindex].end, multiple, 0, this.tasksDataList[Taskindex]?._groupid);
      }
  ...
  async refreshChart() {
    ...
    var timelineHeight = (100 + this.tasksDataList.length * 60) + (this.tasksStartingHeight);
  ...
  async generateNavDate() {
    ...
    if (datesService.isBefore(DateTime.now(), this.datesToShow.start)) {
      ...
      for (var i = 0; i < Difference_In_Days; i++) {
        ...
        this.dates.push({ day: reqdate.day, date: reqdate, month: this.months[reqdate.month-1], isweekend: (reqdate.day == 0 || reqdate.day == 6) ? true : false });
      }
    } else {
      // Find duration between start and end date
      if (datesService.isBefore(this.datesToShow.end, DateTime.now())) {
      ...
      var Difference_In_Days = endDate.diff(this.datesToShow.start, 'days').days;
  ...
  scroll(id: string,index:number) {
    ...
    if(el){
      ...
      document.getElementById('fixed-container-timeline').scrollTo({top: elTop,
        left: elLeft,
        behavior: 'smooth'});
    ...
    columnsData.forEach(column => {
      if (column?.due_date && column?.start_date && column?.project_type) {
        const newColumnsData = {
          data: column,
          differencedays: DateTime.fromISO(column?.due_date).diff(DateTime.fromISO(column?.start_date), 'days').days,
          ...
          taskAfterDueDateStart:this.find_index(DateTime.fromISO(column?.due_date)),
  ...
  async taskAfterDueDate() {
    ...
    for (let index = 0; index < this.projectsdata.length; index++) {
      for (let i = 0; i < this.projectsdata[index].tasks.length; i++) {
        if(datesService.isBefore(DateTime.fromISO(this.projectsdata[index].data.due_date), DateTime.fromISO(this.projectsdata[index].tasks[i].end))) {
          this.projectsdata[index].taskAfterDueDate = DateTime.fromISO(this.projectsdata[index].tasks[i].end).diff(DateTime.fromISO(this.projectsdata[index].data.due_date), 'days').days;
        }
      }
    }
  }
  ...
  async parsedTasks(tasksdata:any) {
    if (!!tasksdata) {
      ...
      function isAlready(id:any) {
        // return already;
        const index = (SortedTask) ? SortedTask.findIndex(t => t._id == id) : -1;
        return index >= 0;
      }
      ...
      SortedTask.map(sortedTask => {
        ...
        if (sortedTask.task.due_to && sortedTask.task.start_date && !(sortedTask.task._parent_task)) {
          this.tasksDataList.push({
            ...
            start: sortedTask.task.is_milestone ? DateTime.fromISO(sortedTask.task?.due_to) : DateTime.fromISO(sortedTask.task.start_date),
  ...
  async min_date(all_dates) {
    ...
    this.sections.forEach(column => {
      if (datesService.isBefore(DateTime.fromISO(column.start_date), min_dtObj)) {
  ...
  find_index(date) {
    const datesService = this.injector.get(DatesService);
    return (!!this.dates) ? this.dates.findIndex(dt => datesService.isSameDay(dt.date, date)) : -1;
  }
  ...
  newTask(post: any, projectId: string) {
    ...
    post.id = post._id;
    this.dateupdate(post, post.task.start_date, post.task.due_date, 1, 1, this.groupData?._id);
  }
}
