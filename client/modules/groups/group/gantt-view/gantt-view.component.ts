...
export class GanttViewComponent implements OnInit, AfterViewInit {
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    var newEndDate = moment(this.tasksdata[Taskindex].end,'YYYY-MM-DD').add(days,'days',);
    var newStartDate = moment(this.tasksdata[Taskindex].start,'YYYY-MM-DD').add(days,'days');
  ...
  onResizeEnd(event: ResizeEvent, Taskid: string, Taskindex): void {
    ...
    if (event.edges?.right) {
      ...
      var newEndDate = moment(this.tasksdata[Taskindex].end,"YYYY-MM-DD").add(multiple,'days');
      this.tasksdata[Taskindex].end = newEndDate.format("YYYY-MM-DD");
      this.tasksdata[Taskindex].task.task.due_to = newEndDate.format("YYYY-MM-DD");
      ...
      var newStartDate = moment(this.tasksdata[Taskindex].start,"YYYY-MM-DD").add(multiple,'days');
      this.tasksdata[Taskindex].task.task.start_date = newStartDate.format("YYYY-MM-DD");
      this.tasksdata[Taskindex].start = newStartDate.format("YYYY-MM-DD");
  ...
  async generateNavDate() {
    if (moment(this.datestoshow.start).isAfter(moment())) {
      // Find duration between start and end date
      const currentDate = moment().format();
      if (moment(this.datestoshow.end).isBefore(moment())) {
        var endDate = moment();
      } else {
        var endDate = moment(this.datestoshow.end);
      }
      ...
      // Find duration between start and end date
      const currentDate = moment(this.datestoshow.start)
      if (moment(this.datestoshow.end).isBefore(moment())) {
        var endDate = moment();
      } else {
        var endDate = moment(this.datestoshow.end);
      }
      var Difference_In_Days = moment(endDate).diff(currentDate,'days');
      ...
      for (var i = 0; i < Difference_In_Days; i++) {
        const reqdate = moment(this.datestoshow.start,"YYYY-MM-DD").add(i,'days');
        this.dates.push({ day: reqdate.date(), date: reqdate, month: this.months[reqdate.month()], isweekend: (reqdate.day() == 0 || reqdate.day() == 6) ? true : false });
      }
    }
  ...
  async parsedTasks(tasksdata) {
    ...
    if (tasksdata.length > 0) {
      ...
      SortedTask.map(x => {
        const startdate: any = moment(x.task.start_date);
        const endate: any = moment(x.task.due_to);
  ...
  async min_date(all_dates) {
    var min_dt = all_dates[0]?.start,
      min_dtObj = moment(all_dates[0]?.start);
    all_dates.forEach(function (dt, index) {

      if (moment(dt.start).isBefore(min_dtObj)) {
        min_dt = dt.start;
        min_dtObj = moment(dt.start);
      }
    });
    return min_dt;
  }
  ...
  async max_date(all_dates) {
    var max_dt = all_dates[0]?.end,
      max_dtObj = moment(all_dates[0]?.end);
    all_dates.forEach(function (dt, index) {

      if (moment(dt.end).isAfter(max_dtObj)) {
        max_dt = dt.end;
        max_dtObj = moment(dt.end);
      }
    });
    return max_dt;
  }
