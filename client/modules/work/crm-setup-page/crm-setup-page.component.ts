...
export class CRMSetupPageComponent implements OnInit, OnDestroy, AfterContentChecked {
	...
	sortedContacts;
	displayedContactsColumns: string[] = ['name', 'company', 'position', 'phone', 'email', 'link', 'star'];
	...
	addNewContactColumn($event: Event) {
		...
		if (index !== -1) {
			...
			this.workspaceData.crm_custom_fields_to_show.push(this.newContactColumnSelected.name);
			...
			this.crmService.saveCRMCustomFieldsToShow(this.workspaceData.crm_custom_fields_to_show)
				.then(res => {
	...
	addNewCompanyColumn($event: Event) {
		...
		if (index !== -1) {
			...
			this.workspaceData.crm_custom_fields_to_show.push(this.newCompanyColumnSelected.name);
	...
	addNewProductColumn($event: Event) {
		...
		if (index !== -1) {
			...
			this.workspaceData.crm_custom_fields_to_show.push(this.newProductColumnSelected.name);
