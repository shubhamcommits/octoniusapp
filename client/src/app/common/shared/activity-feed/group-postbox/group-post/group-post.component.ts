...
export class GroupPostComponent implements OnInit {
  ...
  titleChange(event: any) {
    if (this.edit) {
      if (event === this.title) this.showUpdateDetails = false;
      else {
  ...
  getDate(dateObject: any, property: string) {
    if (property === "start_date") {
      this.startDate = dateObject.toISODate() || null;
    }
    if (property === "due_date") {
      this.dueDate = dateObject.toISODate() || null;
    }
    if (this.edit) {
      this.date.emit({
  ...
  async createPost() {
    ...
    let postData: any = {
      ...
      content:
        !!this.quillData && !!this.quillData.html ? this.quillData.html : "",
      type: this.type,
    ...
    formData.append(
      ...
      (await this.publicFunctions.isShuttleTasksModuleAvailable()).toString()
    );
    formData.append(
      "isIndividualSubscription",
      (await this.publicFunctions.checkIsIndividualSubscription()).toString()
    );
  ...
  onCreatePost(postData: FormData) {
    this.utilityService.asyncNotification(
      $localize`:@@groupCreatePost.pleaseWaitWeCreatingPost:Please wait we are creating the post...`,
      new Promise((resolve, reject) => {
        this.postService
          .create(this.workspaceData?._id, postData)
          .then(async (res) => {
            this.postData = res["post"];
            ...
            resolve(
              this.utilityService.resolveAsyncPromise(
                $localize`:@@groupCreatePost.postCreated:Post Created!`
              )
            );
            this.closeModal();
          })
            ...
            if (err.status === 0) {
              reject(
                this.utilityService.errorNotification(
                  $localize`:@@groupCreatePost.connectionError:Sorry, we are having a hard time connecting to the server. You have a poor connection. The post can't be created.`
                )
              );
            } else {
              reject(
                this.utilityService.rejectAsyncPromise(
                  $localize`:@@groupCreatePost.unableToCreatePost:Unable to create post, please try again!`
                )
              );
            }
          });
  ...
  updateDetails() {
    ...
    if (this.type == "event") {
      ...
      post.date_due_to = this.dueDate.toJSDate();
  ...
  editPost(postId: any, formData: FormData) {
    this.utilityService.asyncNotification(
      $localize`:@@groupCreatePost.pleaseWaitWeUpdatingContent:Please wait we are updating the contents...`,
      new Promise((resolve, reject) => {
        this.postService
          .edit(postId, this.workspaceData?._id, formData)
          .then((res) => {
            ...
            resolve(
              this.utilityService.resolveAsyncPromise(
                $localize`:@@groupCreatePost.detailsUpdated:Details updated!`
              )
            );
          ...
          .catch(() => {
            reject(
              this.utilityService.rejectAsyncPromise(
                $localize`:@@groupCreatePost.unableToUpdatePost:Unable to update the details, please try again!`
              )
            );
          });
      })
  ...
  deletePost() {
    this.utilityService.asyncNotification(
      $localize`:@@groupCreatePost.pleaseWaitWeDeletingPost:Please wait we are deleting the post...`,
      new Promise((resolve, reject) => {
        this.postService
          ...
          .then((res) => {
            ...
            resolve(
              this.utilityService.resolveAsyncPromise(
                $localize`:@@groupCreatePost.postDeleted:Post deleted!`
              )
            );
          })
          .catch((err) => {
            reject(
              this.utilityService.rejectAsyncPromise(
                $localize`:@@groupCreatePost.unableToDeletePost:Unable to delete post, please try again!`
              )
            );
          });
      })
