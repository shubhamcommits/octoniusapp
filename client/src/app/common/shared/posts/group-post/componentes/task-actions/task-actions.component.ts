  ...
  implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
  ...
  constructor(
    ...
    private mdDialogRef: MatDialogRef<
      TaskDialogComponent | NorthStarDialogComponent
    >
  ) {}
  ...
  async ngOnChanges() {
    if (this.postData.type === "task" && this.groupData && this.userData) {
      ...
      await this.publicFunctions
        .getAllUserGroups(this.groupData._workspace)
        .then((groups: any) => {
          groups.forEach(async (group) => {
            group.sections = await this.publicFunctions.getAllColumns(
              group._id
            );
            if (group.sections) {
              group.sections.sort((s1, s2) => (s1.title > s2.title ? 1 : -1));
              this.utilityService
                .removeDuplicates(group.sections, "title")
                .then((sections) => {
                  group.sections = sections;
                });
            }
            if (
              group._id != this.groupData._id &&
              group.sections &&
          ...
          this.userGroups.sort((g1, g2) =>
            g1.group_name > g2.group_name ? 1 : -1
          );
          this.utilityService
            .removeDuplicates(this.userGroups, "_id")
            .then((groups) => {
        ...
        .catch(() => {
          ...
          this.publicFunctions.sendError(
            new Error(
              $localize`:@@taskActions.unableToConnectToServer:Unable to connect to the server, please try again later!`
            )
          );
        });
    }
    ...
    this.subSink.add(
      this.utilityService.currentGroupData.subscribe((res) => {
        if (JSON.stringify(res) != JSON.stringify({})) {
          // Assign the GroupData
          this.groupData = res;

          this.groupMembers = this.groupData._members.concat(
            this.groupData._admins
          );
          this.groupMembers = this.groupMembers.filter((member, index) => {
            return (
              this.groupMembers.findIndex((m) => m._id == member._id) == index
            );
          });
    ...
    if (
      (this.postData?.task?._dependency_task &&
        this.postData?.task?._dependency_task?.length > 0) ||
      (this.postData?.task?._dependent_child &&
        this.postData?.task?._dependent_child?.length > 0)
    ) {
      this.isDependent = true;
    }

    this.estimation =
      (this.postData?.task?.estimation?.hours || "00") +
      ":" +
      (this.postData?.task?.estimation?.minutes || "00");

    this.parentTask = await this.isParent();

    this.shuttleIndex =
      this.groupData?.shuttle_type && this.postData?.task?.shuttles
        ? this.postData?.task?.shuttles.findIndex(
            (s) =>
              (s?._shuttle_group?._id || s?._shuttle_group) !=
              this.groupData?._id
          )
        : -1;
  ...
  async ngOnInit() {
    if (this.groupData?._id || this.postData?._group) {
      this.postService
        .getGroupTemplates(
          this.groupData?._id ||
            this.postData?._group?._id ||
            this.postData?._group
        )
        .then((res) => {
  ...
  ngAfterViewInit() {
    if (this.itemValue == "") this.tasksList = [];
        ...
        .subscribe(async () => {
          ...
          if (this.itemValue == "" && this.dependencyItemValue == "") {
            ...
            if (this.searchingOn === "keyword") {
              this.tasksList =
                (await this.postService.searchPosibleParents(
                  this.groupData._id,
                  this.postData._id,
                  this.itemValue,
                  "subtask"
  ...
  async onUserSelctionEmitter(userId: string) {
    ...
    this.utilityService.asyncNotification(
      $localize`:@@taskActions.pleaseWaitWeAreCloning:Please wait we are cloning the task...`,
      new Promise((resolve, reject) => {
        this.postService
          .cloneToAssignee(assignees, this.postData._id)
          .then((res) => {
            // Close the modal
            ...
            resolve(
              this.utilityService.resolveAsyncPromise(
                $localize`:@@taskActions.taskCloned:👍 Task cloned!`
              )
            );
          })
          .catch((error) => {
            reject(
              this.utilityService.rejectAsyncPromise(
                $localize`:@@taskActions.errorCliningTask:Error while cloning the task!`
              )
            );
          });
      })
  ...
  async isParent() {
    ...
    await this.postService
      .getSubTasksCount(this.postData._id)
  ...
  async copyToGroup(group: string, section: any) {
    ...
    this.utilityService
      .getConfirmDialogAlert(
        $localize`:@@taskActions.areYouSure:Are you sure?`,
        $localize`:@@taskActions.taskWillBeCopied:By doing this the task will be copied to the selected group!`
      )
      .then(async (res) => {
        if (res.value) {
          await this.utilityService.asyncNotification(
            $localize`:@@taskActions.pleaseWaitWeAreCopy:Please wait we are copy the task...`,
            new Promise((resolve, reject) => {
              this.postService
                .transferToGroup(
                  this.postData._id,
                ...
                .then((res) => {
                  ...
                  resolve(
                    this.utilityService.resolveAsyncPromise(
                      $localize`:@@taskActions.taskCopied:👍 Task Copied!`
                    )
                  );
                })
                .catch((error) => {
                  reject(
                    this.utilityService.rejectAsyncPromise(
                      $localize`:@@taskActions.errorWhileCopyingTask:Error while copying the task!`
                    )
                  );
                });
            })
  ...
  async moveToGroup(group: string, section: any) {
    ...
    this.utilityService
      .getConfirmDialogAlert(
        $localize`:@@taskActions.areYouSure:Are you sure?`,
        $localize`:@@taskActions.taskWillBeMoved:By doing this the task will be moved to the selected group!`
      )
      .then(async (res) => {
        if (res.value) {
          await this.utilityService.asyncNotification(
            $localize`:@@taskActions.pleaseWaitWeMoveTask:Please wait we are move the task...`,
            new Promise((resolve, reject) => {
              this.postService
                .transferToGroup(
                  this.postData._id,
                ...
                .then((res) => {
                  ...
                  resolve(
                    this.utilityService.resolveAsyncPromise(
                      $localize`:@@taskActions.taskMoved:👍 Task moved!`
                    )
                  );
                })
                .catch((error) => {
                  reject(
                    this.utilityService.rejectAsyncPromise(
                      $localize`:@@taskActions.errorWhilemovingTask:Error while moving the task!`
                    )
                  );
                });
            })
  ...
  async onTransferPost(data) {
    ...
    if (!isCopy) {
      ...
      if (post.type === "task") {
        this._router.navigate(["/dashboard", "work", "groups", "tasks"], {
  ...
  async setParentTask(parentTaskId: string) {
    this.utilityService
      .getConfirmDialogAlert(
        $localize`:@@taskActions.areYouSure:Are you sure?`,
        $localize`:@@taskActions.taskWillBeChangeToParentTask:By doing this the task will change its parent task!`
      )
      .then((res) => {
        if (res.value) {
          this.postService
            .setParentTask(this.postData._id, parentTaskId)
            .then((res) => {
  ...
  async setDependencyTask(dependencyTaskId: string) {
    this.utilityService
      .getConfirmDialogAlert(
        $localize`:@@taskActions.areYouSure:Are you sure?`,
        $localize`:@@taskActions.taskWillBeChangeToDependencyTask:By doing this the task will change its dependency task!`
      )
      .then((res) => {
        if (res.value) {
          this.postService
            .setDependencyTask(this.postData._id, dependencyTaskId)
            .then((res) => {
              this.postData = res["post"];
  ...
  async removeDependencyTask(index) {
    this.utilityService
      .getConfirmDialogAlert(
        $localize`:@@taskActions.areYouSure:Are you sure?`,
        $localize`:@@taskActions.taskWillBeRemoveItsDependencyTask:By doing this the task will remove its dependency task!`
      )
      .then((res) => {
        if (res.value) {
          this.postService
  ...
  selectTemplate(templatePostId: string, templateName?: string) {
    if (this.templateAction == "save") {
      this.utilityService
        .getConfirmDialogAlert(
          $localize`:@@taskActions.areYouSure:Are you sure?`,
          $localize`:@@taskActions.taskWillChangeContentTemplate:By doing this the task will change the content of the template!`
        )
        .then((res) => {
          if (res.value) {
            this.utilityService.asyncNotification(
              $localize`:@@taskActions.pleaseWaitUpdetingTemplate:Please wait, while we are updating the template for you...`,
              new Promise((resolve, reject) => {
                this.postService
                  .overwriteTemplate(
                    this.postData._id,
                  ...
                  .then((res) => {
                    const index = this.groupTemplates.findIndex(
                      (t) => t._id == templatePostId
                    );
                    if (index >= 0) {
                    ...
                    resolve(
                      this.utilityService.resolveAsyncPromise(
                        $localize`:@@taskActions.templateUpdated:Template updated!`
                      )
                    );
                  })
                  .catch(() =>
                    reject(
                      this.utilityService.rejectAsyncPromise(
                        $localize`:@@taskActions.unexpectedErrorOccuredWhileUpdatingTemplate:An unexpected error occurred while updating the template, please try again!`
                      )
                    )
                  );
              })
      ...
      this.utilityService
        .getConfirmDialogAlert(
          $localize`:@@taskActions.areYouSure:Are you sure?`,
          $localize`:@@taskActions.templateWillBeDeleted:By doing this the template will be deleted!`
        )
        .then((res) => {
          if (res.value) {
            this.utilityService.asyncNotification(
              $localize`:@@taskActions.pleaseWaitDeletingTemplate:Please wait, while we are deleting the template for you...`,
              new Promise((resolve, reject) => {
                this.postService
                  .deletePost(templatePostId)
                  .then((res) => {
                    this.groupTemplates.splice(
                      this.groupTemplates.findIndex(
                        (t) => t._id === templatePostId
                      ),
                      1
                    );
                    resolve(
                      this.utilityService.resolveAsyncPromise(
                        $localize`:@@taskActions.templateDeleted:Template deleted!`
                      )
                    );
                  })
                  .catch(() =>
                    reject(
                      this.utilityService.rejectAsyncPromise(
                        $localize`:@@taskActions.unexpectedErrorOccuredWhileDeletingTemplate:An unexpected error occurred while deleting the template, please try again!`
                      )
                    )
                  );
              })
      ...
      this.utilityService
        .getConfirmDialogAlert(
          $localize`:@@taskActions.areYouSure:Are you sure?`,
          $localize`:@@taskActions.taskWillBeChanged:By doing this the task will change the content of the task!`
        )
        .then((res) => {
          if (res.value) {
            this.utilityService.asyncNotification(
              $localize`:@@taskActions.pleaseWaitUpdatingTask:Please wait, while we are updating the task for you...`,
              new Promise((resolve, reject) => {
                this.postService
                  .createTaskFromTemplate(templatePostId, this.postData._id)
                  .then((res) => {
                    this.taskFromTemplateEmitter.emit(this.postData._id);
                    resolve(
                      this.utilityService.resolveAsyncPromise(
                        $localize`:@@taskActions.taskUpdated:Task updated!`
                      )
                    );
                  })
                  .catch((err) => {
                    ...
                    if (err.status === 0) {
                      reject(
                        this.utilityService.errorNotification(
                          $localize`:@@taskActions.connectionError:Sorry, we are having a hard time connecting to the server. You have a poor connection. The task can't be created.`
                        )
                      );
                    } else {
                      reject(
                        this.utilityService.rejectAsyncPromise(
                          $localize`:@@taskActions.unableToCreatePost:Unable to create task, please try again!`
                        )
                      );
                    }
                  });
  ...
  async createTemplate() {
    const { value: value } = await this.openModal(
      $localize`:@@taskActions.createTemplate:Create Template`
    );
    if (value) {
      this.utilityService.asyncNotification(
        $localize`:@@taskActions.pleaseWaitCreatingTemplate:Please wait, while we are creating the template for you...`,
        new Promise((resolve, reject) => {
          this.postService
            .createTemplate(this.postData._id, this.groupData._id, value)
            .then((res) => {
              this.groupTemplates.unshift(res["post"]);
              this.groupTemplates.sort((t1, t2) =>
                t1.task.template_name > t2.task.template_name ? 1 : -1
              );
              resolve(
                this.utilityService.resolveAsyncPromise(
                  $localize`:@@taskActions.templateCreated:Template created!`
                )
              );
            })
            .catch(() =>
              reject(
                this.utilityService.rejectAsyncPromise(
                  $localize`:@@taskActions.unexpectedErrorOccuredWhileCreatingTemplate:An unexpected error occurred while creating the template, please try again!`
                )
              )
            );
        })
      ...
      this.utilityService.warningNotification(
        $localize`:@@taskActions.templateNameCannotBeEmpty:Template name can\'t be empty!`
      );
    }
  }
  ...
  estimationChange(timeObject) {
    ...
    this.utilityService.asyncNotification(
      $localize`:@@taskActions.pleaseWaitWeAreSavingTask:Please wait we are saving the task...`,
      new Promise((resolve, reject) => {
        this.postService
          .saveEstimation(
          ...
          .then((res) => {
            ...
            resolve(
              this.utilityService.resolveAsyncPromise(
                $localize`:@@taskActions.taskSave:👍 Task saved!`
              )
            );
          })
          .catch((error) => {
            reject(
              this.utilityService.rejectAsyncPromise(
                $localize`:@@taskActions.errorWhileSavingTask:Error while saving the task!`
              )
            );
          });
      })
  ...
  saveTaskType(selectClosed: boolean) {
    if (!selectClosed) {
      this.transformIntoNorthStarEmitter.emit(
        this.taskTypesSelection.includes("northStar")
      );
      this.transformIntoIdeaEmitter.emit(
        this.taskTypesSelection.includes("idea")
      );

      this.postData.task.is_crm_task = this.taskTypesSelection.includes("crm");
      this.transformIntoCRMLeadEmitter.emit(
        this.taskTypesSelection.includes("crm")
      );

      this.postData.task.is_crm_order =
        this.taskTypesSelection.includes("crm_order");
      this.transformIntoCRMOrderEmitter.emit(
        this.taskTypesSelection.includes("crm_order")
      );
    }
  }
