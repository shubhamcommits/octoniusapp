...
export class UserTaskForDayDialogComponent implements OnInit, OnDestroy {
  ...
  async loadTasks() {
    ...
    this.tasksForTheDay = await this.publicFunctions.filterRAGTasks(
      this.tasksForTheDay,
      this.userData
    );
  }
    ...
    if (!!post) {
      ...
      if (dialogRef) {
        ...
        const deleteEventSubs =
          dialogRef.componentInstance.deleteEvent.subscribe((data) => {
  ...
  onDeleteTask(deletedTaskId) {
    ...
    let indexTask = this.tasksForTheDay.findIndex(
      (task: any) => task._id === deletedTaskId
    );
    if (indexTask != -1) {
  ...
  getTaskStatusClass(task: any) {
    return !!task && !!task.task
      ? task.task.status === "to do"
        ...
        : task.task.status.trim() === "completed" ||
          task.task.status.trim() === "done"
