...
export class PostViewComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.flowService
      .getGroupAutomationFlows(this.post._group._id || this.post._group)
      .then((res) => {
        if (res) {
    ...
    this.authToken = `Bearer ${
      this.storageService.getLocalData("authToken")["token"]
    }`;
  }
  ...
  async setPostContent(showAll: boolean) {
    this.postContent = this.utilityService.isJSON(this.post?.content)
      ? await this.publicFunctions.convertQuillToHTMLContent(
          JSON.parse(this.post?.content)["ops"]
        )
      : this.post?.content;
  ...
  eventAssignedToAll(post: any) {
    if (post._assigned_to != null) return post._assigned_to.includes("all");
    else return false;
  }
