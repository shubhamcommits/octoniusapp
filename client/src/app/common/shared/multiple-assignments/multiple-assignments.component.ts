...
export class MultipleAssignmentsComponent implements OnChanges, OnInit {
  ...
  @Input() collection;
  @Input() assigned_to = [];
  @Input() type; // post/flow/filter/chat/portfolio/collection/time-tracking/crm
  @Input() canEdit = true;
  ...
  async ngOnChanges() {
    ...
    if (
      (this.type == "collection" || this.type == "time-tracking") &&
      this.groupId
    ) {
      ...
      this.members = await this.members?.filter((member) => {
        return (
          ["owner", "admin", "manager"].includes(member?.role) && member.active
        );
      });
    } else {
      this.members = await this.workspaceData?.members;
    ...
    if (this.canEdit) {
      if (this.type == "post") {
        this.utilityService.asyncNotification(
          $localize`:@@multipleAssignments.pleaseWaitWeAreUpdatingContents:Please wait we are updating the contents...`,
          new Promise((resolve, reject) => {
            this.postService
              .removeAssigneeFromPost(this.post._id, assigneeId)
              .then((res) => {
                this.post = res["post"];
                const index = this.assigned_to.findIndex(
                ...
                resolve(
                  this.utilityService.resolveAsyncPromise(
                    $localize`:@@multipleAssignments.assigneeRemoved:Assignee removed!`
                  )
                );
              })
              .catch((err) => {
                reject(
                  this.utilityService.rejectAsyncPromise(
                    $localize`:@@multipleAssignments.unableToUpdateDetails:Unable to update the details, please try again!`
                  )
                );
              });
          })
              ...
              new Promise((resolve, reject) => {
                this.postService
                  ...
                  .then((res) => {
                    ...
                    resolve(
                      this.utilityService.resolveAsyncPromise(
                        $localize`:@@multipleAssignments.assigneeAdded:Assignee added!`
                      )
                    );
                  })
                  .catch((err) => {
                    reject(
                      this.utilityService.rejectAsyncPromise(
                        $localize`:@@multipleAssignments.unableToAssignDetails:Unable to assign, please try again!`
                      )
                    );
                  });
              })
