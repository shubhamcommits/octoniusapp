...
export class WorkNavbarComponent
  implements OnInit, AfterContentChecked, OnDestroy
{
  ...
  async ngOnInit() {
    ...
    this.subSink.add(
      this.utilityService.currentWorkplaceData.subscribe((res) => {
    ...
    this.isOrganizationModuleAvailable =
      await this.publicFunctions.isOrganizationModuleAvailable();
    this.isLoungeModuleAvailable =
      await this.publicFunctions.isLoungeModuleAvailable();
  ...
  ngAfterContentChecked() {
    this.subSink.add(
      this.utilityService.routerStateData.subscribe((res) => {
        if (JSON.stringify(res) != JSON.stringify({})) {
          ...
          if (this.routerState == "work") {
            ...
            this.routerState == "people-directory" ||
            this.routerState == "people-directory-chart" ||
            this.routerState == "hive-hr" ||
            this.routerState == "hive-hr-setup" ||
            this.routerState == "hive-hr-timeoff" ||
            this.routerState == "hive-hr-employees"
          ) {
    ...
    this.subSink.add(
      this.utilityService.loungeData.subscribe((res) => {
        this.loungeData = res;
      })
    );
    ...
    this.subSink.add(
      this.utilityService.storyData.subscribe((res) => {
        this.storyData = res;
  ...
  canSeeDashboard() {
    return (
      this.existsElement(this.userData) &&
  ...
  async goBackLounge() {
    if (this.existsElement(this.storyData) && this.storyData._lounge) {
      ...
      this.router.navigate(["/dashboard", "work", "lounge", "details"], {
        queryParams: {
          lounge: loungeId,
        },
      });
      ...
      const loungeId = this.loungeData._parent
        ? this.loungeData._parent._id || this.loungeData._parent
        : null;
      this.publicFunctions.sendUpdatesToLoungeData({});
      this.publicFunctions.sendUpdatesToStoryData({});
  ...
  isUserCRM() {
    return (
      /*this.userData.role == 'owner' || */ this.userData?.crm_role == true
    );
  }
