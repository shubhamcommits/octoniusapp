  ...
  async signIn(workspace: string, email: string, password:string){
    try {
      if(workspace == null || email == null || password == null || workspace == '' || email == '' || password == ''){
        ...
        this.utilityService.asyncNotification('Please wait while we sign you in...', new Promise((resolve, reject)=>{
          let userData = {
          ...
          .subscribe((res)=>{
            ...
            console.log('Error occured while signing in the user', err);
