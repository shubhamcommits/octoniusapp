import { Company, Contact, Group, Product, User, Workspace } from '../models';
...
export class CRMController {
    ...
    async getCRMInfo(req: Request, res: Response) {
        try {
            ...
            const user = await User.findOne({ _id: userId }).select('_workspace').lean();
    ...
    async getCRMContacts(req: Request, res: Response) {
        try {
            ...
            if (!contacts) {
                return sendError(res, new Error('Oops, contacts not found!'), 'Contacts not found, Invalid workspaceId!', 404);
            }
    ...
    async getCRMCompanies(req: Request, res: Response) {
        try {
            ...
            if (!companies) {
                return sendError(res, new Error('Oops, companies not found!'), 'Companies not found, Invalid workspaceId!', 404);
            }
    ...
    async getCRMProducts(req: Request, res: Response) {
        try {
            ...
            if (!products) {
                return sendError(res, new Error('Oops, products not found!'), 'Products not found, Invalid workspaceId!', 404);
            }
    ...
    async removeCRMCustomField(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            const cfIndex = workspace.crm_custom_fields.findIndex(cf => cf._id == fieldId);
            const cf = (workspace && workspace.crm_custom_fields) ? workspace.crm_custom_fields[cfIndex] : null;
