...
export class NotificationsService {
     ...
     */
    async newCommentMentions(actorId: string, ownerId: string, commentId: string, postId: string) {
        try {
     ...
     * @param status
     */
    async taskStatusChanged(postId: String, status: string, actorId: string, ownerId: string, io: any) {
        try {
            if (ownerId) {
                const notification = await Notification.create({
                    _actor: actorId,
    ...
    */
    async newComment(commentId: string, postId: string, actorId: string, ownerId: string) {
        try {
            const notification = await Notification.create({
                _actor: actorId,
                _owner: ownerId,
    ...
    async likeComment(comment: any, ownerId: string, actorId: string) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - New Like on a Post', actor?.first_name + ' ' + actor?.last_name + ' liked your comment on ' + comment._post.title);
                }
            }
        } catch (err) {
            throw err;
    ...
    async followPost(postId: string, ownerId: string, actorId: string) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - New Follow on a Post', actor?.first_name + ' ' + actor?.last_name + ' follows ' + post.title);
                }
            }
        } catch (err) {
            throw err;
    ...
    async likePost(postId, ownerId: string, actorId: string) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - New Like on a Post', actor?.first_name + ' ' + actor?.last_name + ' likes ' + post.title);
                }
            }
        } catch (err) {
            throw err;
    ...
    */
    async postEdited(postId: any, groupId: any, actorId: string, io: any) {
        try {
            // Let usersStream
            let userStream = Readable.from(await User.find({
                _groups: groupId
    ...
    async joinGroup(ownerId: any, groupId: any, actorId: string, io: any) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - Joined Group', actor?.first_name + ' ' + actor?.last_name + ' added you to group ' + group.group_name);
                }
            }
    ...
    */
    async leaveGroup(ownerId: any, groupId: any, actorId: string, io: any) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - Leave Group', actor?.first_name + ' ' + actor?.last_name + ' removed you from group ' + group.group_name);
                }
            }
    ...
    async launchApprovalFlow(item, assigned, posted_by, io) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                const owner = await User.findById({ _id: assigned._id }).select('_workspace integrations.firebase_token').lean();
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - New Approval Flow Launched', actor?.first_name + ' ' + actor?.last_name + ' launched the approval flow on ' + (itemDB.title || itemDB.original_name));
                }
            }
    ...
    async rejectItem(item, assigned, rejected_by, io) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - Approval Flow Rejected', actor?.first_name + ' ' + actor?.last_name + ' rejected the approval flow on ' + (itemDB.title || itemDB.original_name));
                }
            }
    ...
    async itemApproved(item, assigned, io) {
        try {
            ...
            if (process.env.DOMAIN == 'app.octonius.com') {
                ...
                if (owner.integrations.firebase_token) {
                    // Send the notification to firebase for mobile notify
                    firebaseNotifications.sendFirebaseNotification(owner._workspace._id || owner._workspace, owner.integrations.firebase_token, 'Octonius - Approval Flow Rejected', 'Approval flow on ' + (itemDB.title || itemDB.original_name) + ' has been approved');
                }
            }
