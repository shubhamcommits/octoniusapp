...
export class NotificationsController {
    ...
    /**
     * This function is responsible to notifying all the user on re-assigning of a new CRM update to them
     * @param { _id, _assigned_to, _posted_by } post
     */
    async newCRMUpdateNotification(req: Request, res: Response, next: NextFunction) {

        // Fetch Data from request
        const { assigneeId, _assigned_from, companyId, updateId, io } = req.body;
        try {
            // if (_assigned_from != assigneeId) {
                // Call Service function for newTaskReassignment
                await notificationService.newCRMUpdateNotification(assigneeId, _assigned_from, companyId, updateId);
