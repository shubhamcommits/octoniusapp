**PORT 3000** - `http://localhost:3000/api` -> `/api/auths`

- `/`
- `/passwords`

**PORT 4001** - `http://localhost:4001/api` -> `/api/groups`

- `/`
- `/members`
- `/pulse`
- `/columns`
- `/flows`

**PORT 5001** - `http://localhost:5001/api` -> `/api/workspaces`

- `/`
- `/domains`
- `/members`
- `/management`

**PORT 7001** - `http://localhost:7001/api` -> `/api/users`

- `/`

**PORT 8000** - `http://localhost:8000/api` -> `/api/posts`

- `/`
- `/comments`

**PORT 9000** - `http://localhost:9000/api` -> `/api/notifications`

- `/`

**PORT 10000** - `http://localhost:10000/api` -> `/api/utilities`

- `/files`
- `/folders`
- `/uploads`

**PORT 11000** - `http://localhost:11000/api` -> `/api/folio`

- `/`

**PORT 12000** - `http://localhost:12000/api` -> `/api/search`

- `/`

**PORT 13000** - `http://localhost:13000/api` -> `/api/integrations`

- `/`

**PORT 14000** - `http://localhost:14000/api` -> `/api/flamingo`

- `/`

**PORT 15001** - `http://localhost:15001/api` -> `/api/approval`

- `/`

**PORT 16001** - `http://localhost:16001/api` -> `/api/chats`

- `/`
