...
const createHtml = async ( file_uri, res, toFormate="html:HTML:EmbedImages", deleteFile=true) => {
    ...
    exec(`${sofficeCommand} --convert-to ${toFormate} --outdir "./Trash" "${file_uri}"`, (error, stdout, stderr) => {
      if (error) {
      ...
      const htmlFile = "./Trash/"+path.parse(file_uri).name+".html";
      const docxFile = "./Trash/"+path.parse(file_uri).name+".docx";
