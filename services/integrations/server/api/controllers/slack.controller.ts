...
export class SlackController {
    ...
    async slackWebhook(req: Request , res: Response, next: NextFunction) {
        ...
        // If body contain challenge retunr back the challenge
        ...
        const userOctonius = await SlackAuth.findOne({slack_user_id:parsedBodyPayLoad.user.id}).sort({created_date:-1}).populate('_user');
        ...
        // User _id
        const _id = userOctonius['_user']._id;
        
        // Bot Access Token
        botAccessToken = userOctonius['bot_access_token'];

        // Bearer Token
        var BearerToken = "Bearer ";

        // Find user authentication
        const userAuth = await Auth.findOne({_user:_id}).sort({created_date:-1});
        // Token
        ...
        if(!isvalidToken){
            const user = userOctonius['_user']._id;
            const workspace_name = userOctonius['_user'].workspace_name;
            ...
            if(parsedBodyPayLoad.type == "message_action"){
                ...
                try {   
                     ...
                     // User from userOctonius
                    const user = userOctonius['_user'];
    ...
    async isSlackAuth(req: Request , res: Response, next: NextFunction){
        ...
        const slackUserData = await SlackAuth.findOne({_user:req.params.userID}).sort({created_date:-1});
        
        // If SlackData exist user authenticated.
        if(slackUserData && slackUserData!=null){
    ...
    async disconnectSlack(req: Request , res: Response, next: NextFunction){
        ...
        try {
            ...
            // delete all records by slack user id from slakAuth.
            ...
            const updatedUser = await User.findOneAndUpdate({_id:req.params.userID},{$set:{integrations:integration}},{new:true});
