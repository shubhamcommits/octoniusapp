...
export class AuthsController {
    ...
    async checkUserAvailability(email: string) {
        return new Promise(async (resolve, reject) => {
            ...
            const account = await Account.findOne({
                ...
                .populate(
                    "_workspaces",
                    "_id workspace_name workspace_avatar integrations"
                )
                .lean();
            ...
            if (!!account) {
                ...
                if (!!user) {
                    reject({
                        message:
                            "You are already a member of Octonius, please log in!",
     ...
     * @param workspace_name
     */
    async checkUserAvailabilityInWorkspace(
        email: string,
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (user) {
                reject({
                    message:
                        "You are already a member of this workspace, please sign in!",
    ...
    async signUp(req: Request, res: Response, next: NextFunction) {
        ...
        await new AuthsController()
            .checkUserAvailability(userData.email)
            .then(async (resCheck) => {
                if (!!resCheck && !!resCheck["account"]) {
                    ...
                    if (!account) {
                        return sendError(
                            res,
                            new Error(
                                "Unable to create the account, some unexpected error occurred!"
                            ),
                            "Unable to create the account, some unexpected error occurred!",
                            500
                        );
            ...
            .catch((err) => {
                return sendError(
                    res,
                    err,
                    "User is already a member of Octonius!",
                    500
                );
            });
    }
    ...
    async joinWorkspace(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            await new AuthsController()
                .checkUserAvailabilityInWorkspace(
                    accountData.email,
                ...
                .then(async () => {
                    ...
                    if (!workspace) {
                        return sendError(
                            res,
                            new Error(
                                "Workspace does not exist or this email is not allowed to join this workspace!"
                            ),
                            "Workspace does not exist or this email is not allowed to join this workspace!",
                            404
                        );
                        ...
                        return sendError(
                            res,
                            new Error("Your access code is wrong!"),
                    ...
                    if (!accountUpdate) {
                        return sendError(
                            res,
                            new Error(
                                "Unable to update the account, some unexpected error occurred!"
                            ),
                            "Unable to update the account, some unexpected error occurred!",
                            500
                        );
                    ...
                    if (!!user) {
                        ...
                        let groups = workspace.invited_users
                            .filter(
                                (invite) =>
                                    invite.email == accountData.email &&
                                    invite.type == "group"
                            )
                            ...
                            for (let i = 0; i < groups.length; i++) {
                                ...
                                if (!groupUpdate) {
                                    return sendError(
                                        res,
                                        new Error(
                                            `Unable to update the group, some unexpected error occurred!`
                                        ),
                                        `Unable to update the group, some unexpected error occurred!`,
                                        500
                                    );
                        ...
                        if (!user) {
                            return sendError(
                                res,
                                new Error(
                                    "Unable to create the user, some unexpected error occurred!"
                                ),
                                "Unable to create the user, some unexpected error occurred!",
                                500
                            );
                        ...
                        if (!!personalGroup) {
                            return sendError(
                                res,
                                new Error(
                                    "Group name already taken, please choose another name!"
                                ),
                                "Group name already taken, please choose another name!",
                                409
                            );
                    ...
                    notificationsService.createNewUserNotificationForHR(
                        user?._id,
                        workspace?._id
                    );

                    // Generate new token and logs the auth record
                    let token = await auths.generateToken(
                        user,
                        workspace.workspace_name
                    );
                    ...
                    axios.post(
                        `${process.env.MANAGEMENT_URL}/api/mail/join-workspace`,
                        {
                    ...
                    axios
                        .post(`${process.env.MANAGEMENT_URL}/api/user/add`, {
                            API_KEY: workspace.management_private_api_key,
                    ...
                    // Send signup confirmation email
                    axios.post(
                        `${process.env.MANAGEMENT_URL}/api/mail/sign-up`,
                        {
                            user: mailUser,
                        }
                    );
    ...
    async signIn(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (account._workspaces && account._workspaces.length > 0) {
                for (
                    ...
                    if (
                        ...
                        workplace.integrations.is_ldap_connected
                    ) {
                        workplaceLDAPIntegrations = {
                            ...
                            ldap_search_base:
                                workplace.integrations.ldap_search_base,
                            is_ldap_connected:
                                workplace.integrations.is_ldap_connected,
                        };
                    }
            ...
            if (
                workplaceLDAPIntegrations &&
                workplaceLDAPIntegrations.is_ldap_connected
            ) {
                ldapUser = await ldapAuthService.auth(
                    email,
                    password,
                    workplaceLDAPIntegrations
                );
            ...
            // Decrypting Password
            const passDecrypted: any = await passwordHelper.decryptPassword(
                password,
                account["password"]
            );
            ...
            if (!passDecrypted.password) {
                if (
                    ...
                    const passEncrypted: any =
                        await passwordHelper.encryptPassword(password);
                    ...
                    if (!passEncrypted.password) {
                        return sendError(
                            res,
                            new Error(
                                "Unable to encrypt the password to the server"
                            ),
                            "Unable to encrypt the password to the server, please try with a different password!",
                            401
                        );
                    }
                    ...
                    return sendError(
                        res,
                        new Error(
                            "Unable to decrypt the password from the server"
                        ),
    ...
    async selectWorkspace(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            let user = await User.findOne({
                ...
                .select(
                    "_id active email first_name last_name profile_pic workspace_name bio company_join_date current_position role phone_number skills mobile_number company_name _workspace _groups _private_group stats integrations profile_custom_fields hr hr_role"
                )
                .populate({
        ...
        try {
            ...
            if (users.length > 1) {
                ...
                return sendError(
                    res,
                    new Error(
                        "Please enter a valid combination or workspace name and user email or user might be disabled!"
                    ),
                    "Please enter a valid combination or workspace name and user email!",
                    401
                );
            }
        } catch (err) {
        ...
        try {
            ...
            if (!users) {
                return sendError(
                    res,
                    new Error(
                        "Please enter a valid user email or user might be disabled!"
                    ),
                    "Please enter a valid user email or user might be disabled!",
                    401
                );
    ...
    async addUserToSubscription(req: Request, res: Response) {
        try {
            ...
            const user: any = await User.findOne({ _id: req["userId"] }).select(
                "_workspace"
            );
     ...
     * @param workspaceId
     */
    async isNewWorkplacesAvailable(
        req: Request,
        res: Response,
        next: NextFunction
    ) {
        try {
            ...
            await managementService
                .callNewWorkplacesAvailable(environment.toString())
                .then((res) => {
        ...
        try {
            ...
            if (!account) {
                ...
                account = await Account.findByIdAndUpdate(
                    ...
                    .populate(
                        "_workspaces",
                        "_id workspace_name workspace_avatar"
                    )
                    .lean();
            }
    ...
    async getAllWorkspacesIntegrations(
        req: Request,
        res: Response,
        next: NextFunction
    ) {
    ...
    async getAllowedWorkspacesByDomain(
        req: Request,
        res: Response,
        next: NextFunction
    ) {
        try {
            ...
            for (let i = 0; i < workspaces.length; i++) {
                ...
                if (
                    workspace._owner.email !== email &&
                    membersEmails.indexOf(email) < 0
                ) {
    ...
    async getSubscriptionProducts(
        ...
        try {
            let products;
            await authsService.getSubscriptionProducts().then((res) => {
                products = res["data"];
            });
