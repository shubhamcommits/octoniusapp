...
import { isBefore, isSameDay } from "../utils";
...
export class PostService {
    ...
    async getPosts(
        ...
        try {
            ...
            let pinnedQuery = {};
            if (pinned) {
                pinnedQuery = { pin_to_top: true };
            } else {
                pinnedQuery = {
                    $or: [{ pin_to_top: false }, { pin_to_top: null }],
                };
            }
            ...
            const numLikes =
                filters && filters.numLikes ? +filters.numLikes : 0;
    ...
    async getTasksBySection(
        ...
        try {
            ...
            const section = await Column.findById({ _id: sectionId }).populate({
                path: "_group",
                select: "_id shuttle_type",
            });

            if (
                isShuttleTasksModuleAvailable &&
                section?._group?.shuttle_type
            ) {
                posts = await this.filterGroupPosts(
                    Post.find({
                        $or: [
                            ...
                            },
                            {
                                $and: [
                                    { type: "task" },
                                    {
                                        "task.shuttles._shuttle_group":
                                            section?._group?._id,
                                    },
                                    {
                                        "task.shuttles._shuttle_section":
                                            sectionId,
                                    },
    ...
    filterGroupPosts(posts: any, type?: string, numLikes?: number) {
        ...
        if (type == "all" || type == "group")
            filteredPosts = posts
                ...
                .select(
                    "title type permissions task approval_flow_launched tags _group comments_count content _content_mentions created_date files"
                )
        ...
        // If all tasks are selected
            ...
            filteredPosts = posts
                .sort(
                    numLikes && numLikes > 0 ? "-likes_count" : "-task.due_to"
                )
                .select(
                    "title type permissions task approval_flow_launched tags _group created_date files"
                )
                //.populate({ path: '_group', select: this.groupFields })
                .populate({ path: "_posted_by", select: this.userFields })
                .populate({ path: "_assigned_to", select: this.userFields })
            ...
            filteredPosts = posts
                .sort(
                    numLikes && numLikes > 0 ? "-likes_count" : "-created_date"
                )
                .select(
                    "title type permissions task approval_flow_launched tags _group comments_count content _content_mentions created_date files"
                )
    ...
    async populatePostProperties(post: any) {
        if (post.type === "task") {
            ...
            post = await Post.populate(post, [
                ...
                {
                    path: "crm._company",
                    select: "_id name description company_pic",
                },
                {
                    path: "crm._contacts",
                    select: "_id name description phones emails links _company position crm_custom_fields",
                },
                {
                    path: "crm._contacts._company",
                    select: "_id name description company_pic",
                },
            ]);
            ...
            post = await Post.populate(post, [
                {
                    path: "performance_task._assigned_to",
                    select: this.userFields,
                },
                { path: "_group", select: this.groupFields },
                { path: "_posted_by", select: this.userFields },
    ...
    async sendNotifications(post: any, userId: string, logAction?: string) {
        if (!!logAction && logAction == "change_content") {
            return http
                .post(`${process.env.NOTIFICATIONS_SERVER_API}/post-edited`, {
                    postId: post._id,
        ...
        if (post?._content_mentions?.length !== 0) {
            ...
            return http
                .post(`${process.env.NOTIFICATIONS_SERVER_API}/new-mention`, {
                    postId: post._id,
        ...
        switch (post.type) {
            case "task":
                if (post?._assigned_to && post?._assigned_to.length > 0) {
                    ...
                    return http
                        .post(
                            `${process.env.NOTIFICATIONS_SERVER_API}/new-task`,
                            {
                                postId: post?._id,
            ...
            case "event":
                if (post._assigned_to && post._assigned_to.length > 0) {
                    ...
                    return http
                        .post(
                            `${process.env.NOTIFICATIONS_SERVER_API}/new-event`,
                            {
                                postId: post._id,
                                assigned_to: post._assigned_to,
    ...
    async sendNewPostNotification(post: any, userId: string) {
        return http
            .post(`${process.env.NOTIFICATIONS_SERVER_API}/new-post`, {
                postId: post._id,
                groupId: post._group._id || post._group,
                posted_by: post._posted_by,
                userId: userId,
                ...
                case "task":
                    ...
                    postData.task = {
                        due_to: post.date_due_to
                            ? DateTime.fromISO(post.date_due_to)
                            : null,
                        start_date: post.start_date
                            ? DateTime.fromISO(post.start_date)
                            : null,
                        status: post.status,
    ...
    async get(postId: any) {
        ...
        post = await Post.findOne({ _id: postId })
            ...
            .populate({
                ...
                select: "_id title _assigned_to _group task",
            })
            .populate({
                path: "task.shuttles._shuttle_group",
                select: "_id group_name shuttle_type _shuttle_section",
            })
            .populate({
                path: "task.shuttles._shuttle_group",
            ...
            .populate({
                path: "crm.orders._product",
                select: "_id name description crm_custom_fields",
            })
    ...
    async remove(userId: string, postId: string) {
        try {
            ...
            const group: any = await Group.findOne({ _id: post._group })
                .select("_admins")
                .lean();
            const adminIndex = !!group?._admins
                ? group?._admins.findIndex(
                      (admin: any) => (admin._id || admin) == userId
                  )
                : -1;
            // Get user data
            ...
            await Post.deleteMany({
                $and: [{ type: "task" }, { "task._parent_task": postId }],
            });
                ...
                function deleteFiles(files, callback) {
                    ...
                    files.forEach(async (file) => {
                        ...
                        await minioClient.removeObject(
                            (user._workspace + "").toLocaleLowerCase(),
                            file?.modified_name,
                            (error) => {
                                i--;
                                if (error) {
    ...
    async changeTaskDueDate(
        ...
        try {
            ...
            var post: any = await Post.findOneAndUpdate(
                ...
                {
                    "task.due_to": date_due_to
                        ? DateTime.fromISO(date_due_to)
                        : null,
                },
                {
                    new: true,
    ...
    async changeTaskStatus(postId: string, status: string, userId: string) {
        try {
            var oldPost: any = await Post.findOne({ _id: postId }).select(
                "task"
            );
            ...
            if (status !== "to do") {
                http.post(
                    `${process.env.NOTIFICATIONS_SERVER_API}/status-change`,
                    {
                        postId: post._id,
                        userId: userId,
                        assigned_to: post._assigned_to,
            ...
            if (status === "done" || oldPost.task.status === "done") {
                ...
                if (status === "done" && oldPost.task.status !== "done") {
                    ...
                    oldPost.task.status === "done" &&
                    status !== "done"
                ) {
                    counter--;
                }

                this.groupsService.increaseDoneTasks(
                    post._group._id,
                    counter,
                    status
                );
    ...
    async getThisMonthTasks(userId) {
        try {
            ...
            const tasks = await Post.find({
                _assigned_to: userId,
                "task.due_to": {
                    $gte: todayForTask.toJSDate(),
                    $lte: todayPlus30DaysForTask.toJSDate(),
                },
                $or: [
                    { "task.status": "to do" },
    ...
    async getThisWeekTasks(userId) {
        try {
            ...
            const tasks = await Post.find({
                ...
                "task.due_to": {
                    $gte: todayForTask.toJSDate(),
                    $lte: todayPlus7DaysForTask.toJSDate(),
                },
                $or: [
                    { "task.status": "to do" },
                    { "task.status": "in progress" },
                ...
                $or: [
                    {
                        $and: [
                            ...
                            {
                                "event.due_to": {
                                    $gte: todayForEvent.toJSDate(),
                                    $lte: todayPlus30DaysForEvent.toJSDate(),
                                },
                            },
                        ],
                    },
                ...
                $or: [
                    {
                        $and: [
                            ...
                            { _assigned_to: userId },
                            {
                                "event.due_to": {
                                    $gte: todayForEvent.toJSDate(),
                                    $lte: todayPlus7DaysForEvent.toJSDate(),
                                },
                            },
                        ],
                    },
    ...
    async getRecentActivity(userId: any) {
        try {
            var groupList: any = await User.find({ _id: userId }).select(
                "_groups"
            );
    ...
    async getRecentGroups(userId: any) {
        try {
            ...
            var fromComment: any = await Comment.find({
                ...
                .sort("-created_date")
                .limit(2)
                .select("_post._group created_date");
    ...
    async changeCustomFieldValue(
        postId: string,
        userId: string,
        customFieldName: any,
        customFieldTitle: any,
        ...
        try {
            ...
            let post = await Post.findByIdAndUpdate(
                ...
                {
                    $set: {
                        "task.custom_fields": task["task"]["custom_fields"],
                    },
                }
            );
        ...
        try {
            ...
            for (let i = 0; i < posts.length; i++) {
                ...
                const subtasks = await Post.find({
                    ...
                    .select("task.status task.isNorthStar task.northStar")
                    .lean();

                if (subtasks && subtasks.length > 0) {
                    let northStarValues = [];
                    subtasks.forEach((st) => {
                        st.task.northStar.values =
                            st?.task?.northStar?.values?.sort((v1, v2) =>
                                isBefore(
                                    DateTime.fromISO(v1.date),
                                    DateTime.fromISO(v2.date)
                                )
                                    ? 1
                                    : -1
                            );
    ...
    async getNorthStarStats(userId: string) {
        try {
            let nsAssignedToMe = await Post.find({
                $and: [{ "task.isNorthStar": true }, { _assigned_to: userId }],
            })
                .select("task.northStar")
                .lean();

            let nsAssignedByMe = await Post.find({
                $and: [{ "task.isNorthStar": true }, { _posted_by: userId }],
            })
                .select("task.northStar")
                .lean();
    ...
    async getWorspacePostsResults(
        ...
        if (
            filteringGroups &&
            filteringGroups != "undefined" &&
            filteringGroups.length > 0
        ) {
            groups = filteringGroups;
        } else {
            groups = await Group.find({ _workspace: workspaceId })
                .select("_id")
                .lean();
    ...
    async getSubtasks(parentId: any) {
        ...
        posts = await Post.find({
            $and: [{ type: "task" }, { "task._parent_task": parentId }],
        })
            .sort("created_date")
            .populate({ path: "_group", select: this.groupFields })
            .populate({ path: "_posted_by", select: this.userFields })
            ...
            .populate({
                path: "task._parent_task",
                select: "_id title _assigned_to task",
            })
            .populate({
                path: "task.shuttles._shuttle_group",
                select: "_id group_name shuttle_type _shuttle_section",
            ...
            .populate({
                path: "task.northStar.values._user",
                select: this.userFields,
            })
            .populate({ path: "logs._actor", select: this.userFields })
            .populate({
    ...
    async moveToGroup(
        ...
        try {
            ...
            const subtasks = await Post.updateMany(
                {
                    $and: [{ type: "task" }, { "task._parent_task": post._id }],
                },
                {
                    _group: groupId,
                    $push: {
        ...
        try {
            ...
            if (newPost.files) {
                ...
                await files.forEach(async (currentFile: any, index: Number) => {
                    ...
                    let fileName =
                        Date.now().toString() + currentFile.original_name;
    ...
    async setParentTask(postId: string, userId: string, parentTaskId: string) {
        try {
            const parent: any = await Post.findOne({ _id: parentTaskId })
                .select("_group")
                .lean();
    ...
    async setDependencyTask(
        postId: string,
        userId: string,
        dependecyTaskId: string
    ) {
        try {
    ...
    async cloneToAssignee(
        ...
        try {
            ...
            if (newPost.files) {
                ...
                const group: any = await Group.findById(newPost?._group)
                    .select("_workspace")
                    .lean();
                ...
                await files.forEach(async (currentFile: any, index: Number) => {
                    ...
                    await minioClient.bucketExists(
                        group._workspace.toString().toLowerCase(),
                        async (error, exists) => {
                            if (error) {
                                throw error;
                            }
    ...
    async overwriteTemplate(
        ...
        try {
            let template = await Post.findById(templateId).select(
                "_id _group _posted_by"
            );
    ...
    async createTaskFromTemplate(templatePostId: string, newPostId: string) {
        try {
            ...
            const filepath = `${
                newPostId +
                (newPost._group._id || newPost._group) +
                "export" +
                ".docx"
            }`;
            var minioClient = new minio.Client({
            ...
            if (!template.task._parent_task) {
                ...
                (await this.getSubtasks(templatePostId)).forEach((task) => {
                    this.copyToGroup(
                        task._id,
                        template._group._id || template._group,
                        "",
                        null,
                        null,
                        newPostId,
    ...
    async voteIdea(postId: string, vote: number, userId: string) {
        try {
            let post = await Post.findById(postId)
                .select("task.idea.positive_votes task.idea.negative_votes")
                .lean();
    ...
    executeActionFlow(
        ...
        userId: string,
        groupId: string,
        isChildStatusTrigger: boolean
    ) {
        actions.forEach(async (action) => {
            let shuttleIndex = !!post?.task?.shuttles
                ? post?.task?.shuttles?.findIndex(
                      (shuttle) =>
                          (shuttle._shuttle_group._id ||
                              shuttle._shuttle_group) == groupId
                  )
                : -1;
            switch (action.name) {
                case "Assign to":
                    action._user.forEach(async (userAction) => {
                        const assigneeIndex = !!post._assigned_to
                        ...
                        if (assigneeIndex < 0) {
                            if (
                                isChildStatusTrigger &&
                                post.task._parent_task
                            ) {
                                post = await this.addAssignee(
                                    post.task._parent_task._id ||
                                        post.task._parent_task,
                                    userAction,
                                    userId
                                );
                ...
                case "Custom Field":
                    if (isChildStatusTrigger && post.task._parent_task) {
                        post = await this.changeCustomFieldValue(
                            post.task._parent_task._id ||
                                post.task._parent_task,
                            userId,
                            action.custom_field.name,
                            action.custom_field.title,
                ...
                case "Move to":
                    if (isChildStatusTrigger && post.task._parent_task) {
                        if (post?.task?.shuttle_type && shuttleIndex >= 0) {
                            post = await this.selectShuttleSection(
                                post._id,
                                true,
                                action._section._id || action._section,
                                groupId
                            );
                        }
                    } else {
                        if (post?.task?.shuttle_type && shuttleIndex >= 0) {
                            ...
                            if (!post.task._parent_task) {
                                post = await this.changeTaskColumn(
                                    post._id,
                                    action._section._id || action._section,
                                    userId
                                );
                ...
                case "Change Status to":
                    if (isChildStatusTrigger && post.task._parent_task) {
                        if (post?.task?.shuttle_type && shuttleIndex >= 0) {
                            post = await this.selectShuttleStatus(
                                post._id,
                                post?.task?.shuttles[shuttleIndex]
                                    ._shuttle_group?._id ||
                                    post?.task?.shuttles[shuttleIndex]
                                        ._shuttle_group,
                                action.status,
                            ...
                            post = await this.changeTaskStatus(
                                post.task._parent_task._id ||
                                    post.task._parent_task,
                                action.status,
                                userId
                ...
                case "Shuttle task":
                    if (shuttleIndex < 0) {
                        post = await this.selectShuttleGroup(
                            post._id,
                            action?._shuttle_group?._id ||
                                action?._shuttle_group,
                            userId
                        );
                    }
                    break;
                case "Set Due date":
                    if (shuttleIndex < 0) {
                        ...
                        if (action?.due_date_value == "tomorrow") {
                            ...
                            action?.due_date_value == "end_of_next_week"
                        ) {
                            newDueDate = DateTime.now()
                                .plus({ weeks: 1 })
                                .endOf("week")
                        ...
                        if (newDueDate) {
                            post = await this.changeTaskDueDate(
                                post._id,
                                userId,
                                newDueDate
                            );
                ...
                case "Set Estimation Time to":
                    if (shuttleIndex < 0) {
                        post = await this.saveEstimation(
                            post._id,
                            userId,
                            action?.estimation
                        );
    ...
    async triggerToZap(postId: string, userId: string, trigger: string) {
        const post = await Post.findById(postId)
            ...
            .select("task title content tags");
        ...
        let post = await Post.findByIdAndUpdate(
            ...
            },
            {
                $set: {
                    ...
                    "task.shuttles.$[shuttle]._shuttle_section":
                        shuttleSectionId,
                },
            },
            {
    ...
    async selectShuttleStatus(
        ...
        let post = await Post.findByIdAndUpdate(
            ...
            {
                $set: {
                    "task.shuttles.$[shuttle].shuttle_status": shuttleStatus,
                },
            },
            {
                arrayFilters: [{ "shuttle._shuttle_group": shuttleGroupId }],
    ...
    async selectShuttleGroup(
        postId: string,
        shuttleGroupId: string,
        userId: string
    ) {
        let post = await Post.findById({ _id: postId })
            .select("task.shuttles _group")
            .lean();

        const shuttleIndex = (await !!post.task.shuttles)
            ? post.task.shuttles.findIndex(
                  (shuttle) =>
    ...
    //         .populate({ path: 'task.shuttles._shuttle_group', select: '_id group_name shuttle_type _shuttle_section' })
    ...
    async executeAutomationFlows(
        ...
        try {
            if (!isIndividualSubscription) {
                const flows = await this.flowService.getAutomationFlows(
                    groupId
                );
                        ...
                        if (steps && steps.length > 0) {
                            steps.forEach(async (step) => {
                                const childStatusTriggerIndex = !!step.trigger
                                    ...
                                    : -1;
                                const isChildStatusTrigger =
                                    childStatusTriggerIndex >= 0 &&
                                    post.task._parent_task
                                        ? await this.isChildTasksUpdated(
                                              step.trigger[
                                                  childStatusTriggerIndex
                                              ],
                                              post.task._parent_task._id ||
                                                  post.task._parent_task
                                          )
                                        : false;
                                doTrigger = await this.doesTriggersMatch(
                                    step.trigger,
                                    post,
                                    groupId,
                                    isCreationTaskTrigger,
                                    isChildStatusTrigger
                                );
                                const shuttleActionIndex = !!step.action
                                    ? step.action.findIndex(
                                          (action) =>
                                              action.name == "Shuttle task"
                                      )
                                    : -1;
                                doTrigger =
                                    doTrigger &&
                                    (shuttleActionIndex < 0 ||
                                        isShuttleTasksModuleAvailable);
                                if (doTrigger) {
     ...
     * @param parentTaskId
     */
    private async isChildTasksUpdated(trigger: any, parentTaskId: string) {
        let retValue = true;
        if (trigger && parentTaskId) {
            let subtasks = await this.getSubtasks(parentTaskId);
                    ...
                    switch (trigger.name) {
                        case "Assigned to":
                            if (post.task._parent_task) {
                                ...
                                const usersMatch = trigger._user.filter(
                                    (triggerUser) => {
                                        return (
                                            (!!post._assigned_to
                        ...
                        case "Custom Field":
                            if (post.task._parent_task) {
                                ...
                                retValue =
                                    ...
                                    trigger.custom_field.value.toString();
                            }
                            break;
                        case "Section is":
                            if (post.task._parent_task) {
                                if (
                                    ...
                                    const triggerSection =
                                        trigger._section._id ||
                                        trigger._section;
                                    const postSection =
                                        post.task.shuttles[shuttleIndex]
                                            ._shuttle_section._id ||
                                        post.task.shuttles[shuttleIndex]
                                            ._shuttle_section;
                                    retValue =
                                        triggerSection.toString() ==
                                        postSection.toString();
                                } else {
                                    retValue = false;
                                }
                            } else {
                        ...
                        case "Approval Flow is Completed":
                            if (
                                post.approval_active &&
                                post.approval_flow_launched
                            ) {
    ...
    private isApprovalFlowCompleted(flow) {
